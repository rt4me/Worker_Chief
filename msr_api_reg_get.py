...
def get_event_id(event_num, year = datetime.datetime.now().year):
...
def get_registrations(event_uri):
    '''Requests the assignment list for a provided event ID'''

    logging.info('Entering get_registrations')
    ...
    logging.info('Leaving get_registrations')
    logging.debug(f'''get_registrations is returning {response_json['response']['assignments']}''')

    return 'API', response_json['response']['assignments']

#format_reg_info_based_on_source.determin_reg_list_src(get_registrations(get_event_id(5, 2022)))
# print(f'Returned from get_event_id: {get_event_id(5, 2022)}')
# print(f'Organization ID: {os.getenv("ORGANIZATION_ID")}')
# print(get_registrations(get_event_id(5, 2022)))